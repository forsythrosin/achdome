...
void myInitOGLFun()
{
	...
	sgct::ShaderManager::instance()->addShaderProgram("gridShader",
		shaderUtils::absolutePathToShader("gridShader.vert"),
		shaderUtils::absolutePathToShader("gridShader.frag"));
	...
	sgct::ShaderManager::instance()->addShaderProgram("pyramidShader",
		shaderUtils::absolutePathToShader("pyramidShader.vert"),
		shaderUtils::absolutePathToShader("pyramidShader.frag"));
	...
	time_loc = sgct::ShaderManager::instance()->getShaderProgram("pyramidShader").getUniformLocation("time");
	sgct::ShaderManager::instance()->unBindShaderProgram();
}
...
void myPreSyncFun()
{
	if( gEngine->isMaster() ){
		...
		static float pitchRot = 0.0f;
		panRot += (static_cast<float>(mouseDx) * rotationSpeed * static_cast<float>(gEngine->getDt()));
		pitchRot += ((static_cast<float>(mouseDy) * rotationSpeed * static_cast<float>(gEngine->getDt())));
		...
		if(!anyButtonPressed){
			float decaySpeed = 0.02f;
			velocity -= velocity*decaySpeed;
		}

		if(glm::any(glm::greaterThan(glm::abs(velocity), glm::vec3(walkingSpeed)))){
			velocity = glm::normalize(velocity) * walkingSpeed;
		}

		if(spaceBar && pos.y > -0.01){
			std::cout << "whut";
			velocity.y = -12.5;
		}
		...
		pos += velocity * static_cast<float>(gEngine->getDt());
		...
		*/
		...
		xform.setVal( result );
		xDouble.setVal(sgct::Engine::getTime());
	}
}
...
void myEncodeFun()
{
	...
	sgct::SharedData::instance()->writeDouble(&xDouble);
}

void myDecodeFun()
{
	sgct::SharedData::instance()->readObj( &xform );
	sgct::SharedData::instance()->readDouble(&xDouble);
}
...
void keyCallback(int key, int action)
{
	if( gEngine->isMaster() )
	{
		...
		case SGCT_KEY_D:
			...
			break;
		case SGCT_KEY_SPACE:
			spaceBar = ((action == SGCT_REPEAT || action == SGCT_PRESS) ? true : false);
			break;
		}
	}
}
...
void drawPyramid(int index)
{
	...
	glUniform1f(time_loc, xDouble.getVal());
